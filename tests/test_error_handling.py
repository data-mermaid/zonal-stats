...
from app.services.zonal_stats import (ZonalStatsService, ZonalStatsError, 
                                     GeometryError, RasterError, STACError,
                                     get_stac_asset_url)
...
def test_out_of_memory_scenario():
    ...
    # To test memory errors, we need to patch _read_band_data and make sure it's correctly
    # handled by ZonalStatsService
    with patch("app.services.zonal_stats.ZonalStatsService._read_band_data") as mock_read_band:
         
        # Make the _read_band_data method raise a MemoryError
        mock_read_band.side_effect = MemoryError("Not enough memory")
        
        service = ZonalStatsService(
            url=RASTER_PATH, bands=[1], approx_stats=False
        )
...
def test_missing_crs_handling():
    ...
    with patch("rasterio.open") as mock_open, \
         patch("app.services.zonal_stats.ZonalStatsService._transform_geometry_to_raster_crs") as mock_transform:
        ...
        mock_transform.side_effect = RasterError("Source raster has no CRS defined", status_code=400)
...
def test_invalid_geometry_handling():
    ...
    with patch("rasterio.open") as mock_open, \
         patch("app.services.zonal_stats.ZonalStatsService._transform_geometry_to_raster_crs"), \
         patch("app.services.zonal_stats.ZonalStatsService._prepare_geometry") as mock_prepare:
